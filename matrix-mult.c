...
int main(int argc, char *argv[]) {
   ...
   int num_threads = row_a * col_b; // How many threads will there be?
   ...
   // Creates the threads and assigns them a row and column
   int count = 0;
   for (int row_num  = 0; row_num < row_a ; row_num++){
         for (int col_num = 0; col_num < col_b; col_num++){ 
      	      instruction *inst = (instruction *)(malloc(sizeof(instruction)));
      	      inst->row = row_num;
      	      inst->col = col_num; 
      	      inst->rowb = row_b;
       	      pthread_create(&tids[count], &attr, cell_multiply, inst);
	      count ++;
         }
   }   
...
void *cell_multiply(void *param) {
   // Performs the assigned part of the multiply.
   instruction *inst = (instruction *) param; 
   for(int k = 0; k < inst->rowb ; k++){
       matrix_c[inst->row][inst->col]  += matrix_a[inst->row][k] * matrix_b[k][inst->col];
   }     
   // Exits the threads
   pthread_exit(0);
}
