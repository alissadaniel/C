...
void main() {
   printf("ACO350: Advisement - Alissa Daniel\n\n");
...
void *professor(void *param) {
   ...
   while (TRUE) {
      //waiting for a student to be ready
      sem_wait(&studentReady);
      sem_wait(&accessHallSeats);
      numberFreeHallSeats += 1;
      printf("\t\t\t\t\tFree Seats: %d\n", numberFreeHallSeats);
      sem_post(&accessHallSeats);
     
      //letting students know the professor is ready
      sem_post(&professorReady);
     
      printf("Professor: Talking\n");
      //waiting for student to be done
...
void *student(void *param) {
   ...
   while (TRUE) {
      ...
      if(numberFreeHallSeats > 0){
         ...
         //letting the professor know the student is ready
         sem_post(&studentReady);

         //waiting for professor to be ready
         sem_wait(&professorReady);
         printf("Student %d: Talking\n", x->student_id);
         usleep(x->discussion_length);      

         //letting the professor know the student is done  
